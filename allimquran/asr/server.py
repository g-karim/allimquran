...
QURAN_TRANSLATION_CACHE_SECONDS = int(
    os.getenv("QURAN_TRANSLATION_CACHE_SECONDS", str(6 * 24 * 60 * 60))
)
...
def _transcribe(path: str) -> str:
    ...
    try:
        ...
        samples, _ = sf.read(normalized_path, dtype="float32", always_2d=False)
...
def _cached_quran_json(
    ...
    with _mushaf_cache_lock:
        ...
        temporary_path.write_text(json.dumps(data, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
...
async def mushaf_page(page_number: int, mushaf: int = 1):
    ...
    return JSONResponse({"page": page_number, "mushaf": mushaf, "verses": verses, "source": "Quran Foundation Content API"})
...
async def locate_mushaf_verse(surah_number: int, ayah_number: int):
    ...
    data = await run_in_threadpool(_cached_quran_json, f"verse-{surah_number:03d}-{ayah_number:03d}.json", url)
        ...
        words.append(
            {
                ...
                "gloss_en": _plain_text(
                    translation.get("text") if isinstance(translation, dict) else ""
                ),
                "gloss_ru": gloss_ru,
                "gloss_ar": "",
...
async def transcribe(
    ...
    try:
        ...
        transcript = await run_in_threadpool(_transcribe, temporary_path)
        return {"transcript": transcript, "verse_key": verse_key}
    finally:
        await audio.close()
        if temporary_path:
            try:
