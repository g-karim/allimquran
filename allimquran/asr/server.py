...
async def security_headers(request, call_next):
	...
	if request.url.path.startswith("/api/mushaf/") or request.url.path.startswith("/api/quran/words/"):
...
def _cached_quran_json(cache_name: str, url: str) -> dict[str, Any]:
	MUSHAF_CACHE_ROOT.mkdir(parents=True, exist_ok=True)
	cache_path = MUSHAF_CACHE_ROOT / cache_name
	with _mushaf_cache_lock:
		if cache_path.is_file() and time.time() - cache_path.stat().st_mtime < MUSHAF_CACHE_SECONDS:
			try:
