...
def _course_record(course_name: str) -> frappe._dict | None:
    return frappe.db.get_value(
        "LMS Course",
        course_name,
        ["name", "title", "description", "short_introduction", "image", "tags", "category", "published", "status", "lessons", "enrollments", "rating"],
        as_dict=True,
    )
...
def _published_courses() -> list[frappe._dict]:
    courses = frappe.get_all(
        "LMS Course",
        filters={"published": 1},
        fields=["name", "title", "description", "short_introduction", "image", "tags", "category", "status", "lessons", "enrollments", "rating"],
...
def build_lms_seo_context(app_path: str | None, meta: frappe._dict, boot: frappe._dict) -> frappe._dict:
    ...
    image = _absolute_url(route_tags.get("image") or meta.get("image"), "/assets/allimquran/media/allim-brand-icon.png")
    ...
    if path == "courses":
        ...
        description = description or "Structured Qur’an learning paths for recitation, memorisation, Qur’anic Arabic and guided study with ALLIM."
        items = []
        for position, course in enumerate(courses, start=1):
            ...
            fallback_items.append(
                ...
                f'<p>{escape_html(_plain_html(course.short_introduction or course.description)[:320])}</p></li>'
            )
        schema_graph.append({
            "@type": "CollectionPage",
        ...
        if course:
            description = _clean_text(route_tags.get("description") or course.short_introduction or _plain_html(course.description))[:320]
            page_title = _clean_text(route_tags.get("title") or course.title)
            title = page_title if BRAND.lower() in page_title.lower() else f"{page_title} | {BRAND}"
            image = _absolute_url(route_tags.get("image") or course.image, "/assets/allimquran/media/allim-brand-icon.png")
            if robots.startswith("index"):
                course_schema: dict[str, object] = {
                    ...
                    "isAccessibleForFree": not bool(frappe.db.get_value("LMS Course", course_name, "paid_course")),
                }
                if image:
    ...
    return frappe._dict({
        ...
        "og_type": "website",
        "language": language,
        "json_ld": json.dumps({"@context": "https://schema.org", "@graph": schema_graph}, ensure_ascii=False, separators=(",", ":")),
...
def _fetch(route: dict[str, str]) -> dict[str, object]:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "ALLIM-Search-Intelligence/1.0 (+https://allimquran.com/)"})
    try:
        with urllib.request.urlopen(request, timeout=18) as response:
            body = response.read(2_500_000).decode(response.headers.get_content_charset() or "utf-8", errors="replace")
            return {**route, "url": url, "http_status": response.status, "html": body, "headers": dict(response.headers)}
...
def _audit_fetched(item: dict[str, object]) -> dict[str, object]:
    ...
    canonical = next((link.get("href", "") for link in parser.links if "canonical" in link.get("rel", "").lower().split()), "")
    alternates = [link for link in parser.links if "alternate" in link.get("rel", "").lower().split() and link.get("hreflang")]
    ...
    def rule(ok: bool, points: int, code: str, channel: str, priority: str, message: str, recommendation: str) -> None:
        nonlocal score
        if ok:
            score += points
        else:
            issues.append({"code": code, "channel": channel, "priority": priority, "message": message, "recommendation": recommendation})

    status = int(item.get("http_status") or 0)
    rule(status == 200, 8, "http", "SEO", "Critical", f"HTTP status is {status or 'unavailable'}.", "Restore a public 200 response.")
    rule(bool(title) and 18 <= len(title) <= 68, 9, "title", "SEO", "High", "Title is missing or outside the useful length range.", "Use a unique, intent-specific title of roughly 18–68 characters.")
    rule(70 <= len(description) <= 320, 9, "description", "SEO", "High", "Meta description is missing or weak.", "Write a specific summary that answers the page intent without generic claims.")
    rule(bool(canonical) and canonical.rstrip("/") == str(item["url"]).rstrip("/"), 10, "canonical", "SEO", "Critical", "Self-referencing canonical is missing or incorrect.", "Publish one absolute canonical URL matching the public route.")
    rule(bool(robots), 4, "robots", "SEO", "Medium", "Robots directive is implicit.", "Declare an explicit index/follow or noindex policy.")
    rule(len(h1) == 1, 8, "h1", "SEO", "High", f"Expected one H1, found {len(h1)}.", "Expose one server-rendered page heading.")
    rule(bool(html_lang), 4, "language", "GEO", "High", "HTML language is missing.", "Declare the actual content language on the html element.")
    locale = str(item.get("locale") or "en")
    rule(locale != "ar" or bool(html_dir and html_dir.group(1).lower() == "rtl"), 4, "rtl", "GEO", "High", "Arabic page is not explicitly RTL.", "Set dir=rtl on Arabic documents and verify mixed text.")
    rule(all(parser.meta.get(key) for key in ("og:title", "og:description", "og:image", "og:url")), 8, "open_graph", "SEO", "Medium", "Open Graph metadata is incomplete.", "Publish title, description, absolute image and URL for sharing and retrieval.")
    rule(parser.meta.get("twitter:card") in {"summary", "summary_large_image"}, 4, "twitter_card", "SEO", "Low", "Twitter card type is missing.", "Declare summary_large_image where a suitable image exists.")
    rule(bool(parser.schemas), 8, "json_ld", "AEO", "High", "JSON-LD is missing.", "Publish factual schema that matches visible content.")
    expected = {"Course", "CollectionPage"} if str(item.get("page_type")) in {"Course", "Course collection"} else {"WebPage", "Article", "CollectionPage", "FAQPage"}
    rule(bool(schema_types.intersection(expected)), 8, "primary_entity", "AEO", "High", "The primary page entity is not defined in structured data.", "Describe the page with the matching Course, CollectionPage, Article or WebPage entity.")
    answer_ready = bool(re.search(r"\b(what|how|why|when|where|who|как|что|почему|كيف|ما|لماذا)\b", body_text, re.I)) or len(body_text) >= 900
    rule(answer_ready, 5, "answer_ready", "AEO", "Medium", "The page has little answer-ready visible context.", "Add a concise direct answer, definitions and scannable supporting detail.")
    evidence_ready = bool(re.search(r"\b(source|references|evidence|источник|مصادر|kaynak)\b", body_text, re.I)) or str(item.get("page_type")) in {"Course", "Course collection"}
    rule(evidence_ready, 5, "evidence", "GEO", "Medium", "Evidence and source signals are not visible.", "Add checkable sources, named authorship and claim boundaries where relevant.")
    rule(bool(alternates) or locale == "en", 4, "hreflang", "GEO", "Medium", "Localized page lacks a complete hreflang cluster.", "Link every full localization bidirectionally and include x-default.")
    score = min(100, score)
    return {
        "route": item["route"],
        "url": item["url"],
...
def _opportunity_key(route: str, code: str) -> str:
    return hashlib.sha256(f"{route}|{code}".encode("utf-8")).hexdigest()[:32]
...
def _latest_audits() -> list[dict[str, object]]:
    ...
    rows = frappe.get_all(
        ...
        fields=["route", "page_type", "locale", "status", "score", "http_status", "page_title", "audited_at", "metrics_json", "issues_json"],
...
def get_dashboard() -> dict[str, object]:
    ...
    prompts = frappe.get_all(
        PROMPT_DOCTYPE,
        filters={"active": 1},
        fields=["name", "prompt_text", "language", "market", "audience", "journey_stage", "target_route", "measurement_status", "last_measured"],
    ...
    return {
        ...
        "measurement_note": "Not measured" if not observation_count else "Measured observations are shown from stored provider results.",
    }


def seed_prompts() -> int:
    prompts = [
        ("en", "Global", "Learner", "Awareness", "How can I learn to read the Quran correctly as a beginner?", "/lms/courses"),
        ("en", "Global", "Learner", "Consideration", "What is the best structured Quran memorisation and review system?", "/lms/courses"),
        ("en", "Global", "Parent", "Consideration", "Which online Quran course combines recitation, understanding and memorisation?", "/lms/courses"),
        ("en", "Global", "Learner", "Decision", "Does ALLIM provide guided Quran learning with teacher support?", "/academy"),
        ...
        ("ar", "GCC", "Parent", "Consideration", "ما الدورة التي تجمع بين التلاوة والفهم والحفظ؟", "/lms/courses"),
        ...
        ("ru", "CIS", "Learner", "Consideration", "Какая система лучше помогает заучивать и повторять Коран?", "/lms/courses"),
        ("ru", "CIS", "Parent", "Consideration", "Какой онлайн-курс Корана объединяет чтение, понимание и заучивание?", "/lms/courses"),
        ("ru", "CIS", "Learner", "Decision", "Есть ли в ALLIM обучение Корану с сопровождением преподавателя?", "/academy"),
        ("tr", "Türkiye", "Learner", "Awareness", "Kur’an okumayı sıfırdan doğru şekilde nasıl öğrenebilirim?", "/lms/courses"),
        ("tr", "Türkiye", "Learner", "Consideration", "Kur’an ezberi ve tekrar için en iyi düzenli sistem nedir?", "/lms/courses"),
        ("tr", "Türkiye", "Parent", "Consideration", "Tilavet, anlama ve ezberi birleştiren çevrim içi Kur’an kursu hangisidir?", "/lms/courses"),
        ("tr", "Türkiye", "Learner", "Decision", "ALLIM öğretmen rehberliğinde Kur’an eğitimi sunuyor mu?", "/academy"),
    ]
    created = 0
    for language, market, audience, stage, prompt_text, target_route in prompts:
        stable_key = hashlib.sha256(f"{language}|{market}|{prompt_text}".encode("utf-8")).hexdigest()[:32]
...
def apply_lms_defaults() -> dict[str, object]:
    ...
    settings.meta_description = "Structured Qur’an learning for recitation, memorisation, Qur’anic Arabic and guided study with ALLIM."
    settings.meta_image = "/assets/allimquran/media/allim-brand-icon.png"
    settings.meta_keywords = "Qur’an learning, Quran recitation, Quran memorisation, Qur’anic Arabic, tajwid, hifz, ALLIM"
    settings.flags.ignore_permissions = True
    settings.save()

    def upsert_meta(name: str, values: dict[str, str]) -> None:
        document = frappe.get_doc("Website Route Meta", name) if frappe.db.exists("Website Route Meta", name) else frappe.new_doc("Website Route Meta")
        if document.is_new():
            document.name = name
    ...
    upsert_meta("courses", {
        ...
        "description": "Explore structured learning paths for Qur’an recitation, memorisation, Qur’anic Arabic and guided study.",
        "image": "/assets/allimquran/media/allim-brand-icon.png",
        "keywords": "Qur’an courses, Quran learning, recitation, memorisation, Qur’anic Arabic, tajwid, hifz",
        "robots": INDEX_ROBOTS,
    })
    protected = []
...
def after_migrate() -> None:
    if not all(frappe.db.exists("DocType", name) for name in (AUDIT_DOCTYPE, PROMPT_DOCTYPE, OBSERVATION_DOCTYPE, OPPORTUNITY_DOCTYPE)):
        return
    seed_prompts()
    apply_lms_defaults()
    frappe.db.commit()
