(function(){
  ...
  function updateAccountUi(){var button=document.getElementById("account-button");var status=document.getElementById("sync-status");if(currentUser==="Guest")return;var label=currentUser.split("@")[0].replace(/[._-]+/g," ");button.href="/academy";button.querySelector("span").textContent=label;document.getElementById("student-name").textContent=label;status.querySelector("span").textContent=t("signedIn");getJson("/api/method/allimquran.review_portal.access").then(function(data){var access=data.message||{};if(access.profile_url)button.href=access.profile_url;var queue=document.getElementById("review-queue-link");queue.hidden=!access.allowed;queue.querySelector("span").textContent=language==="ar"?"مراجعة التسجيلات":language==="en"?"Review recordings":"Проверка записей"}).catch(function(){/* Keep the website cabinet link if access checks are unavailable. */})}
