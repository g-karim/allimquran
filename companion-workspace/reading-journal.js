...
(function (root) {
  ...
  function commit(next, recovery) {
    try {
      ...
      validate(next); const value = JSON.stringify(next); localStorage.setItem(storageKey, value); raw = value; book = next; failed = false; if (recovery) epoch++; render(); if(root.ALLIMReadingSync) root.ALLIMReadingSync.changed(); return true;
  ...
  function init(config) {
    ...
    if(root.ALLIMReadingSync) root.ALLIMReadingSync.init({getBook:()=>book,validate,setBook:next=>{if(pending)return false;const saved=commit(next,true);if(saved){const form=panel.querySelector('[data-j-settings]');form.elements.past.value=book.past;form.elements.deadline.value=book.deadline;}return saved;}});
  }
  ...
  root.ALLIMReading = { init, recognized, clear: () => { if (pending) { status('error'); return false; } if(root.ALLIMReadingSync&&!root.ALLIMReadingSync.disconnect()){status('error');return false;} const saved = commit(fresh(), true); if (saved && panel) { const form = panel.querySelector('[data-j-settings]'); form.elements.past.value = 0; form.elements.deadline.value = ''; } return saved; } };
