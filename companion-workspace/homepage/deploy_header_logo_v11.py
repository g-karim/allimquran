...
accepted_sources = (
    'src="allim-header-logo.png?v=11"',
    'src="/files/allim-header-logo.png?v=10"',
)
public_source = f'src="{PUBLIC_URL}"'
updated_content = content
if public_source not in updated_content:
    for source in accepted_sources:
        if source in updated_content:
            updated_content = updated_content.replace(source, public_source, 1)
            break
    else:
        raise RuntimeError("Expected previous ALLIM header logo marker was not found")
...
backup_dir = BACKUP_ROOT / f"{timestamp}-allim-header-logo-v11"
