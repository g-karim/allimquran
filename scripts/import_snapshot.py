...
def convert(snapshot, *, keep_media_paths=False):
	baseline_path = CONTENT / "migration_baseline.json"
	baseline = json.loads(baseline_path.read_text()) if baseline_path.exists() else {}
	for group, fields in SOURCE_FIELDS.items():
		for original in snapshot[group]:
			doc = dict(original)
			baseline.setdefault(doc["doctype"] + ":" + doc["name"], fingerprint(original, original))
			...
			for field, extension in fields.items():
				...
				if value is not None:
					...
					if not keep_media_paths:
						value = value.replace("/files/allim-", "/assets/allimquran/media/allim-")
	...
	for doc in snapshot["route_meta"]:
		baseline.setdefault(doc["doctype"] + ":" + doc["name"], fingerprint(doc, doc))
	...
	for doctype, fields in settings.items():
		...
		baseline.setdefault(doctype + ":" + doctype, fingerprint(record, record))
	...
	for original in snapshot["doctypes"]:
		...
		baseline.setdefault(doc["doctype"] + ":" + doc["name"], fingerprint(original, template))
		name = doc["name"].lower().replace(" ", "_")
		write_json(ROOT / "allim_quran" / "doctype" / name / f"{name}.json", doc)
	for original in snapshot.get("desk_pages", []):
		name = original["name"].replace("-", "_")
		path = ROOT / "allim_quran" / "page" / name / f"{name}.json"
		# Keep the native Page definition/controller; do not replace code with DB runtime fields.
		if not path.is_file():
			raise ValueError(f"Copy the native Desk page source before import: {name}")
		baseline.setdefault("Page:" + original["name"], fingerprint(original, json.loads(path.read_text())))
...
if __name__ == "__main__":
	...
	parser.add_argument(
		"--keep-media-paths",
		action="store_true",
		help="Preserve exact live media URLs when adopting reviewed Desk edits before a separate media move",
	)
	args = parser.parse_args()
	convert(json.loads(args.snapshot.read_text()), keep_media_paths=args.keep_media_paths)
